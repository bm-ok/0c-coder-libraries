...
#include <stdint.h>
#include <stddef.h>
extern int onlykey_mldsa_randombytes(uint8_t *out, size_t outlen);
static inline int mld_randombytes(uint8_t *ptr, size_t len)
{
    return onlykey_mldsa_randombytes(ptr, len);
}
#endif
 ...
 *
 */
#define MLD_CONFIG_REDUCE_RAM   /* OnlyKey: ~17KB sign vs ~69KB; fits 64KB RAM */
